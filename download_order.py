...
def generate_dashboard(base_dir: Path) -> Path:
    ...
    for order in orders:
        ...
        map_button = ""
        if bounds:
            map_button = f'<button class="map-btn" onclick="event.stopPropagation(); showMapForOrder(\'{order_id}\', \'{bounds[0][0]},{bounds[0][1]},{bounds[1][0]},{bounds[1][1]}\')">Show Map</button>'

        order_rows.append(f"""
        <tr class="order-row" onclick="toggleOrder('{order_id}')">
            <td><span class="expand-icon" id="icon-{order_id}">+</span> <code>{order_id}</code> {map_button}</td>
            <td>{date_str}</td>
            <td>{order_files}</td>
